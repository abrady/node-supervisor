...
function run (args) {
  var arg, next, watch, program, extensions, executor, debug = null;
  ...
  args = []
  if (debug) {
      sys.debug("adding debug");
    args = args.concat(['--debug='+debug]);
  }
  args = args.concat([program]);

  sys.debug("args: " + args);
  startProgram(args, executor);
...
function startProgram (prog, exec) {
  ...
  child.stdout.on('data', function (chunk) { sys.print(chunk) });
  child.stderr.on('data', function (chunk) { sys.debug(chunk) });
